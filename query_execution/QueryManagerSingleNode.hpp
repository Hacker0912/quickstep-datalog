...
class QueryManagerSingleNode final : public QueryManagerBase {
 ...
 private:
  bool checkNormalExecutionOver(const dag_node_index index,
                                const partition_id part_id) const override {
    return (!workorders_container_->hasNormalWorkOrder(index, part_id) &&
  ...
  bool checkRebuildOver(const dag_node_index index,
                        const partition_id part_id) const override {
    DCHECK(query_exec_state_->hasRebuildInitiated(index, part_id));
    return !workorders_container_->hasRebuildWorkOrder(index, part_id) &&
