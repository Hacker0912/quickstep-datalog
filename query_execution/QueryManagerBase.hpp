...
class QueryManagerBase {
 public:
   ...
   **/
  virtual bool fetchNormalWorkOrders(const dag_node_index index, const partition_id part_id) = 0;
 ...
 protected:
  ...
  bool processRebuild(const dag_node_index index,
                      const partition_id part_id);
   ...
   *         otherwise.
   **/
  inline bool checkAllDependenciesMet(const dag_node_index node_index,
    ...
    for (const dag_node_index dependency_index :
      ...
      if (!checkDependencyMet(node_index, part_id, dependency_index)) {
        return false;
      }
    }
    return true;
  ...
  inline bool checkAllBlockingDependenciesMet(
      const dag_node_index node_index,
      const partition_id part_id) const {
    for (const dag_node_index blocking_dependency_index :
         blocking_dependencies_[node_index]) {
      if (!checkDependencyMet(node_index, part_id, blocking_dependency_index)) {
        return false;
      }
    }
    return true;
   ...
   *         otherwise.
   **/
  inline bool checkOperatorExecutionOver(const dag_node_index index,
                                         const partition_id part_id) const {
    return this->checkNormalExecutionOver(index, part_id) &&
           (!checkRebuildRequired(index) || this->checkRebuildOver(index, part_id));
  }
   ...
   **/
  inline bool checkRebuildInitiated(const dag_node_index index,
                                    const partition_id part_id) const {
  ...
  std::vector<dag_node_index> input_num_partitions_;
  std::unordered_map<dag_node_index, std::size_t> output_num_partitions_;
 ...
 private:
  bool checkNormalExecutionOver(const dag_node_index index) const {
    ...
    return checkNormalExecutionOver(index, 0);
  }

  void markOperatorFinished(const dag_node_index index);

  inline bool checkDependencyMet(const dag_node_index node_index,
                                 const partition_id part_id,
                                 const dag_node_index dependency_index) const {
    if (input_num_partitions_[dependency_index] != input_num_partitions_[node_index]) {
      return query_exec_state_->hasExecutionFinished(dependency_index);
    } else {
      return query_exec_state_->hasExecutionFinished(dependency_index, part_id);
    }
  }

  void processDependentOperators(const dag_node_index index) {
    for (const std::pair<dag_node_index, bool> &dependent_link :
      ...
      for (partition_id dependent_part_id = 0;
        ...
        if (checkAllBlockingDependenciesMet(dependent_op_index, dependent_part_id)) {
          processOperator(dependent_op_index, dependent_part_id, true);
        }
      }
    }
  }
   ...
   *         otherwise.
   **/
  virtual bool checkNormalExecutionOver(const dag_node_index index,
                                        const partition_id part_id) const = 0;
   ...
   **/
  virtual bool initiateRebuild(const dag_node_index index,
                               const partition_id part_id) = 0;
   ...
   **/
  virtual bool checkRebuildOver(const dag_node_index index,
                                const partition_id part_id) const = 0;
