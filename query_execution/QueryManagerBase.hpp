...
class QueryManagerBase {
 public:
  ...
  virtual bool fetchNormalWorkOrders(const dag_node_index index,
 ...
 protected:
  ...
  inline bool checkAllBlockingDependenciesMet(
    ...
    return blocking_dependencies_[node_index][part_id].empty();;
  }

  inline bool checkAllBlockingDependenciesMet(
      const dag_node_index node_index) const {
    for (partition_id part_id = 0; part_id < blocking_dependencies_[node_index].size(); ++part_id) {
      if (!checkAllBlockingDependenciesMet(node_index, part_id)) {
        return false;
      }
    }
    return true;
  ...
  std::vector<std::size_t> input_num_partitions_, output_num_partitions_;
  std::vector<bool> has_repartitions_;

  // For all nodes, store their receiving dependents.
  std::vector<std::unordered_set<dag_node_index>> output_consumers_;

  // For all nodes, store their pipeline breaking dependencies (if any).
  // [dag_node_index][partition_id]
  std::vector<std::vector<std::unordered_set<dag_node_index>>> blocking_dependencies_;
 ...
 private:
  void markOperatorFinishedHelper(const dag_node_index index);

  bool isInputLastPartition(const dag_node_index op_index, const partition_id part_id) const {
    DCHECK_LT(part_id, input_num_partitions_[op_index]);
    return part_id == input_num_partitions_[op_index] - 1;
  }
