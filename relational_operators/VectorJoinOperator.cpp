    ...
    if (probe_relation_is_stored_) {
      ...
      for (std::size_t part_id = 0; part_id < num_partitions_; ++part_id) {
        ...
        for (const block_id probe_block_id : probe_relation_block_ids_[part_id]) {
          container->addNormalWorkOrder(
              new JoinWorkOrderClass(query_id_, build_relation_, probe_relation_, join_key_attribute_,
                                     join_key_attribute_nullable_, part_id, probe_block_id, residual_predicate,
      ...
      for (std::size_t part_id = 0; part_id < num_partitions_; ++part_id) {
        ...
        while (num_workorders_generated_[part_id] < probe_relation_block_ids_[part_id].size()) {
          container->addNormalWorkOrder(
              new JoinWorkOrderClass(query_id_, build_relation_, probe_relation_, join_key_attribute_,
                                     join_key_attribute_nullable_, part_id,
  ...
  if (blocking_dependencies_met_) {
    ...
    if (probe_relation_is_stored_) {
      ...
      for (std::size_t part_id = 0; part_id < num_partitions_; ++part_id) {
        ...
        for (const block_id probe_block_id : probe_relation_block_ids_[part_id]) {
          container->addNormalWorkOrder(
              new VectorOuterJoinWorkOrder(query_id_, build_relation_, probe_relation_, join_key_attribute_,
                                         join_key_attribute_nullable_, part_id, probe_block_id, selection,
...
serialization::WorkOrder* VectorJoinOperator::createNonOuterJoinWorkOrderProto(
  ...
  proto->SetExtension(serialization::HashJoinWorkOrder::join_key_attribute, join_key_attribute_);
  proto->SetExtension(serialization::HashJoinWorkOrder::join_key_attribute_nullable,
