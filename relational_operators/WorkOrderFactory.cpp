...
vector<unique_ptr<WorkOrder>> WorkOrderFactory::ReconstructFromProto(const serialization::WorkOrder &proto,
  ...
  switch (proto.work_order_type()) {
    ...
    case serialization::FINALIZE_AGGREGATION: {
      ...
      for (size_t finalize_part = 0; finalize_part < aggr_state->getNumFinalizationPartitions(); ++finalize_part) {
        work_orders.push_back(make_unique<FinalizeAggregationWorkOrder>(
            query_id, finalize_part, aggr_state,
    ...
    case serialization::INITIALIZE_AGGREGATION: {
      ...
      for (size_t initialize_part = 0;
           initialize_part < aggr_state->getNumInitializationPartitions();
           ++initialize_part) {
        work_orders.push_back(make_unique<InitializeAggregationWorkOrder>(query_id, initialize_part, aggr_state));
      }
      break;
    }
